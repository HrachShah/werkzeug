...
-   ``uri_to_iri`` and ``iri_to_uri`` preserve an empty but present username or
    password instead of dropping it. The truthy ``if parts.username:`` /
    ``if parts.password:`` check treated ``""`` as missing, but
    ``urllib.parse.urlsplit`` distinguishes a present-but-empty component from
    a missing one. :issue:`3189`
