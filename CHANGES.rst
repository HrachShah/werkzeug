...
-   ``uri_to_iri`` and ``iri_to_uri`` preserve an empty but present username or
    password when normalizing URLs, instead of dropping it. Previously, a
    truthiness check silently discarded empty strings, so a URL like
    ``http://:pass@example.com/`` lost the password entirely. :issue:`3189`
